...
def test_class_names(data):
    ...
    assert data['genre'].isin(known_classes).all(), f"Few values in genre class are not valid"
...
def test_column_ranges(data):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        assert data[col_name].dropna().between(minimum, maximum).all(), (
            f"{col_name} is not in the range {minimum, maximum}",
            f"Instead range is between {data[col_name].min()} to {data[col_name].max()}")
